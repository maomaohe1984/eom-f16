 ...
 * we'll use growing to keep track of
 * if our code should be increasing
 * or decreasing the size of the head
 */
...
void draw() {
  if (growing) {
    ...
    // else growing is false
    // so we are shrinking
    headWidth--; // decrement head size
    headHeight--;
    /* -- means
  ...
  // when headWidth gets too big
  // start shrinking
  if (headWidth > 200) {
    growing = false; // we are now shrinking
  }
  if (headWidth < 0){
